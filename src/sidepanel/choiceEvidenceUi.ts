...
function restoreChoice(): void {
  ...
  if (!Array.from(control.options).some((option) => option.value === pendingChoice)) {
    pendingChoice = "";
    updateEvidenceButtonLabel();
    return;
  }
...
export function initChoiceEvidenceUi(): void {
  ...
  document.getElementById("selected-text")?.addEventListener(
    "click",
    rememberChoice,
    true,
  );
  ...
  if (root) {
    const observer = new MutationObserver(() => queueMicrotask(restoreChoice));
