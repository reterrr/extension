...
function isLocal(object: LegacyStoredObject): boolean {
  return (
    !!activePageUrl &&
    (object.sourceUrl === activePageUrl ||
      state.rules.some(
        (rule) =>
          rule.objectId === object.id && rule.pageUrl === activePageUrl,
      ))
  );
}

/**
 * workspace.js owns object switching. Rebuild the same switcher order and use
 * aria-current from its DOM so this module stays independent of its closure.
 */
function chosenObject(): LegacyStoredObject | undefined {
  ...
  const local = state.objects.filter(isLocal);
  const saved = state.objects.filter((object) => !isLocal(object));
  const ordered = [...local].reverse().concat([...saved].reverse());
  return ordered[activeIndex];
}

function geographyLabel(value: string): string {
  return (
    BurbotGeography.catalog.find((entry) => entry.value === value)?.label ?? value
  );
}
...
function renderSearch(object: LegacyStoredObject): void {
  ...
  const catalog = BurbotGeography.catalog.filter(
  ...
  if (!catalog.length) {
    help.textContent =
      "Ten typ nie ma jeszcze lokalnego słownika. Podregiony, powiaty i gminy zostaną zasilone ze słownika SQLite/TERYT.";
    return;
  }

  help.textContent = search.value
    ? `${matches.length} wyników`
